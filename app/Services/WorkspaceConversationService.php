...
class WorkspaceConversationService
{
    ...
    public function startConversation(int $userId, ?int $workspaceId = null): array
    {
        ...
        if ($workspaceId) {
            ...
            if ($workspace && $workspace->is_supported) {
            ...
            if ($workspace) {
                return $this->reply($conversation, self::MSG_UNDER_DEV);
            }
        }
    ...
    {
        $workspace = $this->matchWorkspace($text);
    ...
    protected function matchWorkspace(?string $text): ?Workspace
    {
        if (blank($text)) {
            return null;
        }
